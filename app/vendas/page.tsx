  ...
  return (
    <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0, overflow: "hidden" }}>
      <div style={{
        ...
        background: "radial-gradient(circle at 30% 30%, #9B59F5aa, #4A0FAD44, transparent 70%)",
      ...
      <div style={{
        ...
        background: "radial-gradient(circle at 60% 40%, #E879F944, #7C3AED33, transparent 70%)",
      ...
      <div style={{
        ...
        background: "radial-gradient(circle at 40% 60%, #F59E0B33, #E879F922, transparent 70%)",
...
function Card3D({ children, style }: { children: React.ReactNode; style?: React.CSSProperties }) {
  ...
  return (
    <div
      ...
      style={{
        transform: `perspective(1000px) rotateX(${rotate.x}deg) rotateY(${rotate.y}deg) scale(${hover ? 1.02 : 1})`,
...
function CheckoutModal({ plan, onClose }: { plan: typeof plans[0]; onClose: () => void }) {
  ...
  const input: React.CSSProperties = {
    width: "100%", background: "#160830", border: "1px solid #2D1458",
    borderRadius: 10, padding: "12px 14px", fontSize: 14,
    color: "#fff", fontFamily: "sans-serif", outline: "none", boxSizing: "border-box",
  };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#000000cc", backdropFilter: "blur(16px)", zIndex: 999, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: "linear-gradient(160deg, #160830, #0F0520)", border: "1px solid #2D1458", borderRadius: 24, padding: 32, maxWidth: 420, width: "100%", position: "relative", fontFamily: "sans-serif", boxShadow: "0 40px 80px #00000088" }}>
        <button onClick={onClose} style={{ position: "absolute", top: 16, right: 16, background: "#2D145833", border: "none", color: "#9478C0", cursor: "pointer", fontSize: 18, width: 32, height: 32, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
        ...
        {done ? (
          <div style={{ textAlign: "center", padding: "20px 0" }}>
            ...
            <a href="/dashboard" style={{ display: "block", background: "linear-gradient(135deg, #7C3AED, #A855F7)", color: "#fff", borderRadius: 14, padding: "14px", fontSize: 15, fontWeight: 700, textDecoration: "none" }}>
              Acessar plataforma →
            </a>
          </div>
        ) : (
            ...
            {step === 2 && (
              <div>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
                  {[["pix", "⚡ PIX"], ["card", "💳 Cartão"], ["boleto", "📄 Boleto"]].map(([id, label]) => (
                    <button key={id} onClick={() => setMethod(id)}
                      style={{ flex: 1, background: method === id ? "#7C3AED33" : "#160830", color: method === id ? "#A855F7" : "#9478C0", border: `1px solid ${method === id ? "#7C3AED" : "#2D1458"}`, borderRadius: 10, padding: "10px 4px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "sans-serif" }}>
                      {label}
                    </button>
                  ))}
                </div>
                ...
                <button onClick={handlePay} disabled={loading}
                  style={{ width: "100%", background: "linear-gradient(135deg, #7C3AED, #A855F7)", color: "#fff", border: "none", borderRadius: 12, padding: "15px", fontSize: 15, fontWeight: 700, cursor: "pointer", fontFamily: "sans-serif", boxShadow: "0 8px 24px #7C3AED44" }}>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, padding: "16px 32px", display: "flex", alignItems: "center", justifyContent: "space-between", background: "#07010Fcc", backdropFilter: "blur(20px)", borderBottom: scrollY > 50 ? "1px solid #2D1458" : "1px solid transparent", transition: "border-color 0.3s" }}>
        ...
        <div style={{ display: "flex", gap: 28, alignItems: "center" }}>
          {[["funcionalidades", "Funcionalidades"], ["planos", "Planos"], ["faq", "FAQ"]].map(([id, label]) => (
            <button key={id} onClick={() => scrollTo(id)} style={{ background: "none", color: "#9478C0", border: "none", fontSize: 14, cursor: "pointer", fontFamily: "sans-serif", transition: "color 0.2s" }}
              onMouseEnter={e => (e.currentTarget.style.color = "#fff")}
              onMouseLeave={e => (e.currentTarget.style.color = "#9478C0")}>
              {label}
            </button>
          ))}
          <button onClick={() => setSelectedPlan(plans[0])} style={{ background: "linear-gradient(135deg, #7C3AED, #A855F7)", color: "#fff", border: "none", borderRadius: 10, padding: "10px 22px", fontSize: 14, fontWeight: 700, cursor: "pointer", boxShadow: "0 4px 16px #7C3AED44" }}>
            Começar grátis
          </button>
        </div>
      </nav>
        ...
        <div style={{ textAlign: "center", maxWidth: 780 }}>
          ...
          <div style={{ opacity: 0, animation: "fadeUp 0.8s ease 0.4s forwards" }}>
            <h1 style={{ fontSize: "clamp(42px, 7vw, 76px)", fontWeight: 900, color: "#fff", margin: "0 0 24px", lineHeight: 1.0, letterSpacing: "-0.05em" }}>
              Capture leads.<br />
              <span style={{ background: "linear-gradient(90deg, #A855F7, #E879F9, #F59E0B)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", display: "inline-block" }}>
                Feche contratos.
              </span>
            </h1>
          </div>
          ...
          <div style={{ opacity: 0, animation: "fadeUp 0.8s ease 0.8s forwards", display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap" }}>
            <button onClick={() => setSelectedPlan(plans[0])}
              style={{ background: "linear-gradient(135deg, #7C3AED, #A855F7)", color: "#fff", border: "none", borderRadius: 16, padding: "18px 40px", fontSize: 17, fontWeight: 800, cursor: "pointer", boxShadow: "0 8px 40px #7C3AED66", letterSpacing: "-0.01em", position: "relative", overflow: "hidden" }}>
              <span style={{ position: "relative", zIndex: 1 }}>Começar grátis agora →</span>
              <div style={{ position: "absolute", inset: 0, background: "linear-gradient(135deg, #9B59F5, #E879F9)", opacity: 0, transition: "opacity 0.3s" }}
                onMouseEnter={e => ((e.currentTarget as HTMLElement).style.opacity = "1")}
                onMouseLeave={e => ((e.currentTarget as HTMLElement).style.opacity = "0")} />
            </button>
            <button onClick={() => scrollTo("planos")}
              style={{ background: "transparent", color: "#EDE9FE", border: "1px solid #2D1458", borderRadius: 16, padding: "18px 32px", fontSize: 16, fontWeight: 600, cursor: "pointer", backdropFilter: "blur(10px)" }}>
              Ver planos
            </button>
          </div>
          ...
          <div style={{ opacity: 0, animation: "fadeUp 0.8s ease 1s forwards", marginTop: 80, display: "flex", justifyContent: "center", gap: 60, flexWrap: "wrap" }}>
            {[["4800", "+", "leads capturados"], ["320", "+", "formulários ativos"], ["2400000", "R$", "em vendas geradas"]].map(([n, suffix, label]) => (
              <div key={label} style={{ textAlign: "center" }}>
                <div style={{ fontSize: 32, fontWeight: 900, color: "#fff", letterSpacing: "-0.04em" }}>
                  {suffix === "R$" ? "R$ " : ""}<Counter end={parseInt(n)} />{suffix !== "R$" ? suffix : ""}
                </div>
                <div style={{ fontSize: 13, color: "#9478C0", marginTop: 4 }}>{label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <Reveal>
            <h2 style={{ fontSize: "clamp(28px, 4vw, 42px)", fontWeight: 900, color: "#fff", textAlign: "center", margin: "0 0 16px", letterSpacing: "-0.04em" }}>
              ...
              <span style={{ color: "#EF4444" }}>deixando na mesa?</span>
            </h2>
            <p style={{ color: "#9478C0", textAlign: "center", fontSize: 16, margin: "0 0 56px" }}>
              Se você não qualifica seus leads, está perdendo tempo e dinheiro.
            </p>
          </Reveal>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 16 }}>
            {[
              { icon: "😩", title: "Liga pra todo mundo", desc: "Perde horas com leads frios que nunca vão comprar. Energia desperdiçada.", color: "#EF4444" },
              { icon: "📉", title: "Sem critério de qualidade", desc: "Não sabe qual lead tem potencial real. Fecha menos do que poderia.", color: "#F59E0B" },
              { icon: "🤯", title: "Dados espalhados", desc: "Instagram, DM, WhatsApp, planilha... você não sabe onde estão seus clientes.", color: "#E879F9" },
            ].map((p, i) => (
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <Reveal>
            <div style={{ textAlign: "center", marginBottom: 60 }}>
              <div style={{ display: "inline-block", background: "#22C55E22", color: "#22C55E", border: "1px solid #22C55E33", borderRadius: 20, padding: "4px 16px", fontSize: 13, fontWeight: 700, marginBottom: 20 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 20, marginBottom: 40 }}>
            {[
              { icon: "🤖", title: "Score de IA por lead", desc: "Cada lead recebe pontuação automática de 0 a 100. Foque só nos quentes.", gradient: "linear-gradient(135deg, #7C3AED22, #E879F911)", border: "#A855F744" },
              { icon: "💬", title: "WhatsApp com 1 clique", desc: "Abra o WhatsApp do lead direto do dashboard. Zero fricção, mais vendas.", gradient: "linear-gradient(135deg, #25D36622, #22C55E11)", border: "#25D36644" },
              { icon: "📱", title: "Integração Instagram Ads", desc: "Conecte ao Meta Ads Manager e capture leads dos anúncios sem link externo.", gradient: "linear-gradient(135deg, #F59E0B22, #E879F911)", border: "#F59E0B44" },
            ].map((f, i) => (
          ...
          <Reveal>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 12 }}>
              {[
                { icon: "⚡", title: "Formulário em 5 min" },
                { icon: "🔍", title: "Filtros inteligentes" },
                { icon: "📊", title: "Dashboard completo" },
                { icon: "📥", title: "Exportar CSV/Excel" },
                { icon: "🔗", title: "Link universal" },
                { icon: "📈", title: "Relatórios avançados" },
              ].map(f => (
                <div key={f.title} style={{ background: "#0F0520", border: "1px solid #2D1458", borderRadius: 14, padding: "16px 14px", textAlign: "center", transition: "border-color 0.2s, transform 0.2s" }}
            ...
            {[
              ...
              <Reveal key={t.name} delay={i * 150}>
                <Card3D style={{ background: "linear-gradient(160deg, #160830, #0F0520)", border: "1px solid #2D1458", borderRadius: 20, padding: 24, height: "100%", boxSizing: "border-box" as React.CSSProperties["boxSizing"] }}>
                  <div style={{ color: "#F59E0B", fontSize: 18, marginBottom: 14, letterSpacing: 2 }}>★★★★★</div>
        ...
        <div style={{ maxWidth: 960, margin: "0 auto" }}>
          <Reveal>
            <div style={{ textAlign: "center", marginBottom: 48 }}>
              <div style={{ display: "inline-block", background: "#A855F722", color: "#A855F7", border: "1px solid #A855F744", borderRadius: 20, padding: "4px 16px", fontSize: 13, fontWeight: 700, marginBottom: 20 }}>
                Planos e preços
              </div>
            ...
            {plans.map((plan, i) => {
              ...
              return (
                <Reveal key={plan.id} delay={i * 150}>
                  <Card3D style={{
                    background: plan.highlight ? "linear-gradient(160deg, #1E0845, #160830)" : "#0F0520",
                    border: `2px solid ${plan.highlight ? "#A855F7" : "#2D1458"}`,
                    borderRadius: 24, padding: 28, position: "relative",
                    boxShadow: plan.highlight ? "0 0 60px #7C3AED33, 0 0 0 1px #A855F722" : "none",
                  }}>
                    {plan.highlight && (
                      <div style={{ position: "absolute", top: -16, left: "50%", transform: "translateX(-50%)", background: "linear-gradient(90deg, #7C3AED, #E879F9)", borderRadius: 24, padding: "5px 20px", fontSize: 12, fontWeight: 800, color: "#fff", whiteSpace: "nowrap", boxShadow: "0 4px 16px #7C3AED66" }}>
                        {plan.tag}
                      </div>
                    )}
                    {!plan.highlight && (
                      <div style={{ display: "inline-block", background: plan.color + "22", color: plan.color, borderRadius: 20, padding: "2px 12px", fontSize: 12, fontWeight: 700, marginBottom: 12 }}>{plan.tag}</div>
                    )}
                      ...
                      {plan.features.map(f => (
                        <div key={f.text} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          <span style={{ width: 20, height: 20, borderRadius: "50%", background: f.ok ? "#22C55E22" : "#9478C018", display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                            {f.ok
                              ? <svg width="10" height="10" viewBox="0 0 10 10"><path d="M2 5l2.5 2.5L8 3" stroke="#22C55E" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" fill="none" /></svg>
                              : <svg width="8" height="8" viewBox="0 0 8 8"><path d="M2 2l4 4M6 2L2 6" stroke="#9478C0" strokeWidth="1.5" strokeLinecap="round" /></svg>
                            }
                          </span>
              ...
              <Reveal key={i} delay={i * 80}>
                <div onClick={() => setOpenFaq(openFaq === i ? null : i)}
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16 }}>
                    ...
                    <span style={{ color: "#A855F7", fontSize: 22, transform: openFaq === i ? "rotate(45deg)" : "none", transition: "transform 0.3s", flexShrink: 0 }}>+</span>
                  </div>
                  {openFaq === i && (
                    <div style={{ marginTop: 14, fontSize: 14, color: "#9478C0", lineHeight: 1.8, borderTop: "1px solid #2D1458", paddingTop: 14 }}>{f.a}</div>
                  )}
                </div>
              </Reveal>
            ))}
      ...
      <section style={{ padding: "100px 32px", textAlign: "center", position: "relative", zIndex: 1 }}>
        ...
        <Reveal>
          <div style={{ position: "relative", maxWidth: 600, margin: "0 auto" }}>
            <h2 style={{ fontSize: "clamp(32px, 5vw, 52px)", fontWeight: 900, color: "#fff", margin: "0 0 20px", letterSpacing: "-0.04em", lineHeight: 1.1 }}>
              Pronto para fechar mais?
            </h2>
            <p style={{ color: "#9478C0", fontSize: 18, margin: "0 0 40px", lineHeight: 1.6 }}>
              Comece grátis agora. Sem cartão de crédito.
            </p>
            <button onClick={() => setSelectedPlan(plans[0])}
              style={{ background: "linear-gradient(135deg, #7C3AED, #A855F7, #E879F9)", color: "#fff", border: "none", borderRadius: 18, padding: "20px 48px", fontSize: 18, fontWeight: 900, cursor: "pointer", boxShadow: "0 12px 48px #7C3AED66", letterSpacing: "-0.02em", transition: "transform 0.2s, box-shadow 0.2s" }}
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = "scale(1.04)"; (e.currentTarget as HTMLElement).style.boxShadow = "0 20px 60px #7C3AED88"; }}
              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = "scale(1)"; (e.currentTarget as HTMLElement).style.boxShadow = "0 12px 48px #7C3AED66"; }}>
      ...
      <footer style={{ borderTop: "1px solid #2D1458", padding: "28px 32px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16, position: "relative", zIndex: 1 }}>
        ...
        <div style={{ display: "flex", gap: 20 }}>
          {["Termos", "Privacidade", "Suporte"].map(l => (
            <span key={l} style={{ fontSize: 13, color: "#9478C0", cursor: "pointer", transition: "color 0.2s" }}
